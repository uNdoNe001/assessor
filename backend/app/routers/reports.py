...
def generate_report(
    ...
    if assessment_id:
        ...
        q = select(
            func.sum(models.Answer.maturity * models.Question.weight).label("wsum"),
            func.sum(models.Question.weight).label("w")
        ).join(models.Question, models.Question.id == models.Answer.question_id
        ).where(models.Answer.assessment_id == assessment_id)
