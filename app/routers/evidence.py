...
@router.post("/upload")
def upload_evidence(client_id: int = Form(...), file: UploadFile = File(...), db: Session = Depends(get_db), user=Depends(require_role(["pss_owner","pss_analyst","client_admin","client_contributor"]))):
    uid = str(uuid.uuid4())
...
@router.get("/download")
def download(token: str, user=Depends(require_role(["pss_owner","pss_analyst","client_admin","client_readonly","auditor_readonly"]))):
    try:
